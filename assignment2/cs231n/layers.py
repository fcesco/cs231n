...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    var = np.sum((x-u_b)**2, axis = 0)/N
    x_hat = (x-u_b)/np.sqrt(var + eps)
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for n in xrange(N):
      ...
      for f in xrange(F):
          for h_p in xrange(H_p):
              for w_p in xrange(W_p):
                  h1 = h_p * stride
                  h2 = h_p * stride + HH
                  w1 = w_p * stride
                  w2 = w_p * stride + WW
      ...
      for f in xrange(F):
          for h_p in xrange(H_p):
              for w_p in xrange(W_p):
                  ...
                  w1 = w_p * stride
                  w2 = w_p * stride + WW
                  dx_pad[:, h1:h2, w1:w2] += w[f,:,:,:] * dout[n,f,h_p,w_p]
                  dw[f,:,:,:] += x_pad[:, h1:h2, w1:w2] * dout[n,f,h_p,w_p]
  ...
  for n in xrange(N):
      for h in xrange(H_p):
          for w in xrange(W_p):
              ...
              out[n,:,h,w] = np.max(window.reshape((C, pool_h*pool_w)), axis=1)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
      ...
      for c in xrange(C):
          for h in xrange(H_p):
              for w in xrange(W_p):
                  ...
                  dx[n,c,h1:h2,w1:w2] = np.reshape(window3, (pool_h, pool_w)) * dout[n,c,h,w]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
